...
@if($fs->status==0)
<h4>Round 2 :  <p style="color:red" id="demo1"></p></h4>
@endif
...
<table class="table table-responsive">
 ...
 <td>{{ $topic->category_name }}</td>
   ...
   <td>{{ $fs->video_url }}</td>
    ...
    @if($fs->status==1)
      <td style="color:green">Congatulations!!you are selected for round 3</td>
      @elseif($fs->status==2)
      <td style="color:yellow">You did'nt qualify for  round 3</td>
      @else
      ...
      @if($fs->status==1)
      <?php
        $checkround2 = DB::table('round3storytbls')->where('userid',session('cid'))->first();
       ?>
      @if($checkround2)
      <td><a href="/round3story" class="btn btn-outline-secondary text-black">Round 3</a></td>
      @else
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Round 3</h5>
      ...
      <div class="modal-body">
        ...
        <h5>Terms And Conditions</h5>
            <?php
            $desc = strip_tags($thirdround->description);
            ?>
            {{$desc}}
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <a id="round3" href="/round3story_get" class="btn btn-outline-secondary text-black">Write Story</a>
        <button id="round3show" style="display:none" class="btn btn-outline-secondary text-black" onclick="alert('round3 ended')">Write Story</button>
      </div>
    </div>
  </div>
</div>
     @endif
      ...
      @endif
      <?php
        $checkuser = DB::table('round2storytbls')->where('userid',session('cid'))->first();
       ?>
       @if($checkuser)
       @if($fs->status==1)
   ...
   @else
    <td><a href="/editround2story/{{$fs->id}}" class="btn btn-success text-white">Update</a></td>
    <td><a href="/deleteround2story/{{$fs->id}}" class="btn btn-danger text-white">Delete</a></td>
    <td><a href="/round2storydetail/{{$fs->id}}" class="btn btn-warning text-white">Detail</a></td>
    @endif
    @endif
  </tr>
  </tbody>
...
<input type="hidden" name="" id="datevalue" value="{{$thirdround->date}}">
